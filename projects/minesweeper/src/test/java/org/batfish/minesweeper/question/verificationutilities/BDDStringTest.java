...
public class BDDStringTest {
    ...
    @Before
    public void setup() {
        tbdd = new TransferBDD(new ConfigAtomicPredicates(
                List.of(),
                Set.of(CommunityVar.from("10:10"),
    ...
    public void onlyCommunities() {
        ...
        assertEquals("comm(50:50)",BDDString.get(tbdd,commBDDString("50:50").and(commBDDString("50:50"))));

        // conjunctions
        assertEquals("comm(20:20),comm(30:30)",BDDString.get(tbdd,commBDDString("20:20").and(commBDDString("30:30"))));
        ...
        assertEquals("False",BDDString.get(tbdd,commBDDString("40:40").and(commBDDString("40:40").not())));

        // disjunctions
        String u = BDDString.get(tbdd,commBDDString("20:20").or(commBDDString("30:30")));
        assertEquals("comm(20:20) OR comm(30:30)",u);
        assertEquals("True",BDDString.get(tbdd,commBDDString("40:40").or(commBDDString("40:40").not())));
        String v = BDDString.get(tbdd,commBDDString("20:20").or(commBDDString("30:30")).or(commBDDString("50:50").not()));
        assertEquals("!comm(50:50) OR comm(20:20) OR comm(30:30)",v);

        // implications
        String w = BDDString.get(tbdd,(commBDDString("40:40").and(commBDDString("50:50"))).imp(commBDDString("10:10")));
    ...
    public void onlyPrefixes() {
        ...
        String c = BDDString.get(tbdd,prefixStringToBDD("25.13.0.0/16",true)
                .or(prefixStringToBDD("12.0.0.0/8",true)));
        assertEquals("prefix(12.0.0.0/8) OR prefix(25.13.0.0/16)",c);
        String d = BDDString.get(tbdd,prefixStringToBDD("25.13.0.0/16",true)
                .or(prefixStringToBDD("12.0.0.0/8",false)));
        assertEquals("!prefix(12.0.0.0/8)",d);
        String e = BDDString.get(tbdd,prefixStringToBDD("25.13.7.0/24",false)
                .or(prefixStringToBDD("120.0.0.0/8",false)));
        assertEquals("True",e);
...
//        assertEquals("",h);
    ...
    public void includesCommunitiesAndPrefixes() {
        ...
        BDD e = (commBDDString("20:20").and(commBDDString("30:30"))).and(prefixStringToBDD("25.13.0.0/16",false));
        ...
        assertEquals("!comm(30:30),comm(40:40),prefix(25.13.0.0/16) OR !comm(40:40)," +
                "!prefix(25.13.0.0/16),comm(30:30) OR comm(30:30),comm(40:40)",f_str);
    }
}
